...
#include"../include/NetAlgorithm.h"
...
public:
    ...
    int RunNet(cv::Mat&left_image,cv::Mat&right_image,float*pointcloud,cv::Mat&disparity);
    int Release();
...
NetAlgorithm::NetAlgorithm(/* args */)
{
}

NetAlgorithm::~NetAlgorithm()
{
}


int NetAlgorithm::Initialize(char* model_path,int gpu_id,char*calibration_path)
{
...
int NetAlgorithm::RunNet(cv::Mat&left_image,cv::Mat&right_image,float*pointcloud,cv::Mat&disparity)
{
    ...
    raftstereo.RunNet(left_image,right_image,pointcloud,disparity);
    return 0;    
}

int NetAlgorithm::Release()
{
    return raftstereo.Release();
}

void NetAlgorithm::ReadObjectYml(const char* filename, CalibrationParam&Calibrationparam)
{
...
int NetAlgorithm::RectifyImage(cv::Mat&Image_src,cv::Mat&rectifyImageL2,cv::Mat&rectifyImageR2)
{
...
int NetAlgorithm::RectifyImage(cv::Mat&rectifyImageL2,cv::Mat&rectifyImageR2)
{
    //RectifyImage
    cv::Size s1, s2;
...
{
    NetAlgorithm*raftstereo_algorithm=new NetAlgorithm();
...
//Params		img			
//Return        int          
int Net(void* p,cv::Mat&left_image,cv::Mat&right_image,float*pointcloud,cv::Mat&disparity)
{
    NetAlgorithm*raftstereo_algorithm=(NetAlgorithm*)p;
    return raftstereo_algorithm->RunNet(left_image,right_image,pointcloud,disparity);
}
